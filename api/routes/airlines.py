...
from utils.load_queries import load_report, AIRLINE_REPORTS
...
def list_reports():
    ...
    for report_id, report_name in AIRLINE_REPORTS.items():
        reports.append({
            "report_id": report_id,
            "report_name": report_name,
            "url": f"/api/airlines/{report_id}"
        })
...
def get_report(report_id: str):
    ...
    """
    if report_id not in AIRLINE_REPORTS:
        raise HTTPException(
            status_code=404,
            detail="Report not found"
        )

    report_name = AIRLINE_REPORTS[report_id]
